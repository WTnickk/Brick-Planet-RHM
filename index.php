...
require_once($_SERVER['DOCUMENT_ROOT']."/../private/header.php");
	if (!$AUTH) {
			...
			<div class="grid-container site-container-margin">
				<div class="grid-x grid-margin-x">
					<div class="large-7 cell">
						<div class="lb-title">Welcome to Brick Planet!</div>
						<div class="lb-text">Brick Planet is an online 3D gaming platform where users can collaborate to create awesome games, clothing, and participate in a virtual economy. Join us to create an online virtual world where the possibilities are endless.</div>
					</div>
					<div class="large-4 large-offset-1 cell">
						<div class="landing-signup">
							...
							<form action="'.$serverName.'/sign-up/" method="POST">
								...
								<div class="g-recaptcha" data-sitekey="6Lea0S4UAAAAALb2kbdI4NAWskF_czwfusCWwvfR"></div>
								<div class="push-15"></div>
		...
		echo '
		<script>
		document.title = "Dashboard - Brick Planet";
		</script>
		<div class="grid-x grid-margin-x">
			<div class="large-4 cell">
				<div class="dashboard-user-splash-color relative" style="background:#3766A3;">
					<a href="'.$serverName.'/users/'.$myU->Username.'/">
						<div class="dashboard-user-avatar-thumb" style="background-image:url('.$cdnName . $myU->AvatarURL.'-thumb.png);background-size:cover;"></div>
					</a>
				</div>
				...
				';
					...
					if (!$cache->get('blog')) {
						$feed = file_get_contents('https://blog.brickplanet.com/rss/');
						$cache->set('blog', $feed, 120);
					} else {
						$feed = $cache->get('blog');
					}
					...
					foreach ($xml->channel->item as $entry) {
						...
						if ($i > 1) {
							echo '<div class="blog-card-divider"></div>';
						}
						
						echo '
						<div class="grid-x grid-margin-x align-middle blog-card">
							<div class="shrink cell">
								<i class="fa fa-rss-square" aria-hidden="true"></i>
							</div>
							<div class="auto cell">
								<a href="'.$entry->link.'" target="_blank"><div class="blog-title">'.$entry->title.'</div></a>
								<div class="blog-text-preview">'.LimitTextByWords(strip_tags($entry->description), 18).'</div>
							</div>
						</div>
						';
